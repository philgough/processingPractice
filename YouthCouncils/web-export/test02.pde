...
void groupChange () {
  if (frameCount % _groupChanger == 1) {
    _xGroup = int(floor(random(_w)));
    _yGroup = int(floor(random(_h)));
  }
}
...
class Cell {
  ...
  void render() {
    ...
    for (int i = 0; i <= 360; i += (360/_numPoints)) {
      vertex(sin(radians(i)) * _gridSize/2.5, cos(radians(i)) * _gridSize/2.5);
    }
    endShape(CLOSE);
    fill(360, 80);
